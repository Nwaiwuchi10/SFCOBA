...
const AdminGetBroadcast = () => {
  ...
  const postDelete = (id, e) => {
    ...
    setLoadings(true);
    axios
      .delete(`https://sfcoba.herokuapp.com/api/announcement/delete/${id}`)
  ...
  return (
    <AdminLayout>
      <h3 className="text-center mb-4 mt-4">
        View All {poster.length} BroadCast Post{" "}
      </h3>
      <section class="intro">
        {loading ? (
          <Loader />
          ...
          <table class="table">
            ...
            <tbody>
              {poster?.map((usery) => (
                <>
                  <tr key={usery._id}>
                    <td data-label="S.Caption">{usery?.caption}</td>

                    <td data-label="S.Content"> {usery.content}</td>

                    <td data-label="S.edith/delete">
                      <Link to={`/edithBroadcast/${usery._id}`}>
...
export default AdminGetBroadcast;
